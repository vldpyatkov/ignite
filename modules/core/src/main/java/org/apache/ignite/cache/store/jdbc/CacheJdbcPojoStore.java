...
public class CacheJdbcPojoStore<K, V> extends CacheAbstractJdbcStore<K, V> {
    ...
    private Object buildPojoObject(@Nullable String cacheName, String typeName,
        ...
        try {
            ...
            for (JdbcTypeField fld : flds) {
                ...
                ClassProperty prop = mc.props.get(fldJavaName);
    ...
    @Override protected void prepareBuilders(@Nullable String cacheName, Collection<JdbcType> types)
        ...
        Map<String, PojoPropertiesCache> pojoMethods = U.newHashMap(types.size() * 2);
        ...
        for (JdbcType type : types) {
            ...
            if (keyKind == TypeKind.POJO) {
                ...
                pojoMethods.put(keyTypeName, new PojoPropertiesCache(keyTypeName, type.getKeyFields()));
            }
            ...
            if (valKind == TypeKind.POJO)
                pojoMethods.put(valTypeName, new PojoPropertiesCache(valTypeName, type.getValueFields()));
        }

        if (!pojoMethods.isEmpty()) {
            Map<String, Map<String, PojoPropertiesCache>> newPojosMethods = new HashMap<>(pojosProps);

            newPojosMethods.put(cacheName, pojoMethods);

            pojosProps = newPojosMethods;
        }
    }
    ...
    private static class ClassProperty {
        ...
        private Object get(Object obj) throws IllegalAccessException, InvocationTargetException {
            ...
            throw new IllegalAccessException("No such property!");
        }
